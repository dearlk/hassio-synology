...
class Notify(hass.Hass):
  ...
  def initialize(self):
    self.set_state("sensor.appd_notify",state="Appdaemon started !",attributes={"announce":False})
  ...
  def send_notify(self, entity, attribute, old, new, kwargs):
    ...
    if tts_trigger and announcement_mode == "on" and  sleep_mode=="off":
      # self.call_service('script/turn_on', entity_id='script.livingroom_tv_mute')
      ...
      # self.run_in(self.call_service('script/turn_on', entity_id='script.livingroom_tv_mute'), 5)
      # #self.call_service('script/turn_on', entity_id='script.tv_mute')
      self.call_service("python_script/notify",message=new, announce=1, override=1)
