...
class TrackerForJyoti(hass.Hass):
  ...
  def initialize(self):
    ...
    self.log("##############################################################################################")
    self.listen_state(self.left_home,    self.home_tracker, old="home",         new="not_home",     duration=120)
    self.listen_state(self.reached_home, self.home_tracker, old="not_home",     new="home",         duration=120)
...
#####################################################################################################################################
  def track_state_change (self, entity, attribute, old, new, kwargs): 
    if old != new and new != "not_home" and new != "home":
      if self.get_state(self.home_tracker) != "home":
...
#####################################################################################################################################
  def left_home (self, entity, attribute, old, new, kwargs): 
    #ignore false alarms due to wifi connectivity once at home
    if self.get_state(self.is_at_home) == "on" or self.now_is_between ("20:00:00","09:00:00"):
      return
...
#####################################################################################################################################
  def reached_home (self, entity, attribute, old, new, kwargs): 
    #ignore false alarms due to wifi connectivity once at home
    if self.get_state(self.is_at_home) == "off" or self.now_is_between ("20:00:00","09:00:00"):
      return
