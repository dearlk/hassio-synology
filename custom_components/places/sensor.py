...
class Places(Entity):
    ...
    def __init__(self, hass, devicetracker_id, name, api_key, options, home_zone, map_provider, map_zoom):
        ...
        self._entity_picture = "/local/"+self._name + ".jpg"
    ...
    def do_update(self, reason):
        ...
        if hasattr(self, '_devicetracker_id'):
            ...
            now = datetime.now()
            #_LOGGER.debug("now.tzinfo={}",now.tzinfo)
            #now = datetime.now().astimezone(pytz.timezone('Asia/Singapore'))
        ...
        if proceed_with_update:   
            ...
            #current_time = as_timestamp(now()) | timestamp_custom("%I:%M") 
